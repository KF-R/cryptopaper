...
## TODO: shift orc stats to reddit/u/Shopro
import os
...
def fetch_bbc_news(headline_count=4, timeout=TIMEOUT):
    ...
    for x in range(headline_count):
        results.append(headlines[x+1].text.strip())
    return(results)
...
def pygame_loop(stop_event):
    ...
    while running:
        ...
        # Show status (including elapsed time if < SECONDS_PER_CANDLE * MAX_CANDLES)
        elapsed = ''
        ...
        # Chart Frame
        pygame.draw.rect(display, 0, pygame.Rect( 3, CHART_TOP-4, WIN_W-6, CHART_BOTTOM-CHART_TOP+8), 6, 3 )
        ...
        print_at(display, (ORC_DAYS * CHART_COL_W) + 36, CHART_BOTTOM + 106, f"{orc_figures[-1]:,.0f}", 60)
        print_at(display, (ORC_DAYS * CHART_COL_W) + 36, CHART_BOTTOM + 88, f"High: {max(orc_figures):,.0f}", 16)
        print_at(display, (ORC_DAYS * CHART_COL_W) + 48, CHART_BOTTOM + 174, f"Low: {min(orc_figures):,.0f}", 16)
